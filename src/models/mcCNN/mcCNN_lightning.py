...
class mcCNNModule(pl.LightningModule):
    def __init__(self, 
                input_dim: int = 1000,
                output_dim: int = 3,
        ...
        super(mcCNNModule, self).__init__()

        # create model instance
        self.model = mcCNN(input_dim=input_dim,
    ...
    def on_validation_epoch_end(self):
        # skip logging is we are in sanity checking
    ...
    def calculate_physio_params(self, all_x, all_y, all_y_hat, all_lengths):
        ...
        for x, pt, it, pp, ip in zip(all_x, true_p, true_i, pred_p, pred_i):
            x = x[~np.isnan(x)]
            ap_true = x[pt] - x[it] if x[pt] > x[it] else x[it] - x[pt]
            ap_pred = x[pp] - x[ip] if x[pp] > x[ip] else x[ip] - x[pp]
